      ...
      <div className="bg-white rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 flex items-center">
              ...
              {t('trips.detailedWeatherForecast')}
            </h2>
        ...
        {isLoading ? (
          ...
          <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
            ...
            <p className="text-red-800 text-lg font-medium">{t('trips.failedToLoadWeather')}</p>
            <p className="text-red-600 text-sm mt-2">{t('trips.pleaseTryAgainLater')}</p>
          </div>
        ) : weather ? (
          <div className="space-y-6">
            {/* Main weather display */}
            <div className="bg-linear-to-br from-blue-500 via-blue-600 to-teal-600 rounded-3xl p-8 text-white relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/20 rounded-xl p-4">
                    <p className="text-white/70 text-sm">{t('trips.humidity')}</p>
                    <p className="text-white text-2xl font-bold">{weather.humidity}%</p>
                  </div>
                  <div className="bg-white/20 rounded-xl p-4">
                    <p className="text-white/70 text-sm">{t('trips.feelsLike')}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-blue-50 rounded-xl p-4 text-center border border-blue-100">
                <p className="text-blue-600 text-sm font-medium mb-1">{t('trips.windSpeed')}</p>
                <p className="text-blue-900 text-xl font-bold">15 km/h</p>
              </div>
              <div className="bg-green-50 rounded-xl p-4 text-center border border-green-100">
                <p className="text-green-600 text-sm font-medium mb-1">{t('trips.visibility')}</p>
                <p className="text-green-900 text-xl font-bold">10 km</p>
              </div>
              <div className="bg-purple-50 rounded-xl p-4 text-center border border-purple-100">
                <p className="text-purple-600 text-sm font-medium mb-1">{t('trips.uvIndex')}</p>
          ...
          <div className="text-center py-12">
            <p className="text-gray-500">{t('trips.noWeatherData')}</p>
          </div>
        )}
